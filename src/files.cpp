...
vector<boost::filesystem::path> get_images(string directory, vector<string> extensions = vector<string>()) {
    boost::filesystem::path boost_directory(directory);
    vector<string> extensions_filter;
    vector<boost::filesystem::path> images;

    // throw error
    if (!boost::filesystem::is_directory(directory)) {
        throw invalid_argument("Directory: " + directory + " is not a directory.");
    }
    ...
    for(auto& entry : boost::make_iterator_range(boost::filesystem::directory_iterator(boost_directory), {})) {
    ...
    if (images.size() == 0) {
        throw "No images found in the directory: " + directory;
    }
